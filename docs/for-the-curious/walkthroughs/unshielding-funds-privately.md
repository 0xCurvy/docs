...
In the previous scenario we covered the first of the three actions of the **Aggregator.sol**: shielding.

Now we are going to need to use the other two: aggregations and unshielding.
...
### ⚫ Curvy
3. The **Curvy SDK** on Bob's side next submits aggregation requests which merge multiple notes together, and a subsequent unshielding request.
...
> [!IMPORTANT]
> From the perspective of an on-chain observer, funds have simply moved from the **Aggregator.sol** to the address of the CEX. The information about the previous owner of the funds, and the notes themselves that got aggregated into a single note for unshielding is completely hidden.
