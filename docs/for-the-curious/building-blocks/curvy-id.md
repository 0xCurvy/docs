...
Curvy user is identified by the following tuple:
...
User's BJJ public key is used when we want to address a new Note in aggregation in the Privacy Aggregator.

### Viewing public key

Viewing public key is necessary to construct proper stealth addressees and notes of the user.

Corresponding viewing private key can be shared by the user with a 3rd party to gain access into the trail of private transactions that would otherwise be completely hidden.
The private key is envisioned to be shared when the user just wants to prove his transactional activity, but doesn't want to give control over his funds.

### Spending public key

This public key is necessary to construct proper stealth addresses and Portal recovery addresses for the user.

Explained in more detail in the [Compliance](../compliance/index.md) section, the spending public key is used to recover funds not deemed compliant by Curvy whose entry was blocked into the Privacy Aggregator.

The recovery of Portal funds is a completely transparent on-chain action, and other than returning non-compliant funds is also used when an unsupported token is sent to a Portal address.  


## How are Curvy user's private keys generated?

Curvy user's private keys are generated by deriving them from a deterministic source.
...
or signature points of a message hashed with a well-known salt and a user supplied password produced by signing with a 3rd party crypto wallet.
